...
class FastDeconv2D(tf.keras.layers.Layer):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding='valid', dilation_rate=1,
        ...
        super(FastDeconv2D, self).__init__(
            in_channels=2, out_channels, kernel_size, stride, dilation_rate=dilation_rate,
...
def UNet_ConvUnit(input_tensor, stage, nb_filter, kernel_size=3, mode='None'):   
    x = FastDeconv2D(in_channels=2,out_channels=nb_filter, kernel_size=(3,3), padding='same',activation='selu')(input_tensor)
    x = FastDeconv2D(in_channels=nb_filter,out_channels=nb_filter, kernel_size=(3,3), padding='same',activation='selu')(x)
